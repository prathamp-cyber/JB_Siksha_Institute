document.addEventListener('DOMContentLoaded', () => {
    ...
    // 7. Contact Form Handler (Redirect to WhatsApp)
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', (e) => {
            ...
            const courseLabel = (courseSelect && courseSelect.options[courseSelect.selectedIndex]) 
                ? courseSelect.options[courseSelect.selectedIndex].text 
                : courseValue;
            const message = document.getElementById('message').value.trim();

            if (!name || !email || !phone || !courseValue) {
                if (formMessage) {
            ...
            let text = `Hello JB Siksha Institute, my name is ${name}.\n`;
            text += `Phone: ${phone}\n`;
            text += `Email: ${email}\n`;
            text += `Interested Field: ${courseLabel}`;
            if (message) {
                text += `\nMessage: ${message}`;
            }

            // Redirect to WhatsApp
            const whatsappUrl = `https://wa.me/918780459636?text=${encodeURIComponent(text)}`;
